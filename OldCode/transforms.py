...
class CSVToImages:
    ...
    def __call__(self,video_frames):
            # open video file
            video_file = video_frames["video_file"]
            frame_ids = video_frames["frame_ids"]
            ...
            frames = torch.FloatTensor(self.channels, height, width)

            # read frame
            cap.set(cv2.CAP_PROP_POS_FRAMES, frame_ids)
            ret, frame = cap.read()
            if ret:
                # successfully read frame
                # BGR to RGB
...
class CSVToVideos:
    def __init__(self, padding_mode="last", sampling_mode="uniform", num_frames=8):
        self.channels = 3
        self.padding_mode = padding_mode
    ...
    def __call__(self, video_frames):
            ...
            # open video file
            video_file = video_frames["video_file"]
            start_frame = video_frames["start_frame"]
            end_frame = video_frames["end_frame"]
            ...
            frames = torch.FloatTensor(self.channels, self.num_frames, height, width)
            frame_ids = utils.sample_frames(self.sampling_mode, start_frame, end_frame, self.num_frames) 
            #print(self.sampling_mode)
            #print("Start frame: ", start_frame, " End frame: ", end_frame, " ", frame_ids)
            for index in range(len(frame_ids)):

                # read frame
                cap.set(cv2.CAP_PROP_POS_FRAMES, frame_ids[index])
                ret, frame = cap.read()
                if ret:
                    # successfully read frame
                    # BGR to RGB
...
class ReadTransformedVideos:
    ...
    def __call__(self, video_frames):
        # open video file
        #print(video_frames)
        frames = torch.load(video_frames)
...
class ReadTransformedImages:
    ...
    def __call__(self, video_frames):
        #print(video_frames)
        #print(video_frames)
        frames = torch.load(video_frames["video_file"])[:,:,video_frames["frame_ids"],:,:]
        #print(frames.size())
